...
\textbf{Re: Regular Paper submission, ``FAIR-X: A Threshold-Transfer
Evaluation Contract for Behavioral Malware Detection under Distribution
Shift''}
...
(TDSC). The submission comprises an 18-page main manuscript and a separate
four-page supplementary file. The paper addresses a concrete dependability
problem in
security-machine-learning systems: a detector can rank malicious processes
well yet produce fragile operational decisions when its validation-derived
threshold expands under a shifted deployment distribution.
...
It is directly relevant to TDSC because it studies the evidence conditions
under which a learned security component supports a reproducible alerting decision. The paper
...
Published security-ML guidance warns against data leakage, underpowered baselines, and
inappropriate metrics. However, these principles still leave the need for
...
FAIR-X closes this gap through:
\begin{itemize}
    \item a general threshold-transfer evaluation contract that fixes data access, feature fitting,
    ...
    falsifiable;
    \item repeated cross-fitted threshold selection over all available real
    BETH development positives;
    \item target-copy-free GRU/LSTM capacity ablations that bound the
    sequence-model conclusion; and
    \item an instantiation on BETH and independent protocol validation on MalBehavD-V1, supplemented by
...
contain zero malicious processes, whereas 121 of 198 official-test processes
are malicious. Under one common validation-FPR policy, TabRF and RF-500 obtain F1 scores of
0.7210 and 0.7043, while MetaRF obtains 0.6756; the hybrid fusion
therefore remains below the direct tabular operating point. Fifty thresholds fixed by
...
low-F1 across 23\,666--241\,074 parameters (AUC 0.4238--0.4987), so the paper
limits this bounded result to the tested objectives and representation. On
MalBehavD-V1, the same evaluation contract yields repeated-split median F1
0.9533, supporting protocol portability while direct cross-schema model
transfer remains a separate evidence target.

\textbf{Claim boundaries and reproducibility.}
The manuscript bounds its claims to behavioral malicious-process detection,
threshold-transfer evaluation, and protocol portability. Ransomware-family
attribution, calibrated posterior risk, direct BETH-to-MalBehavD model
transfer, and readiness for autonomous containment remain separate evidence
targets because of the available labels, schemas, test size, and
early-detection results. The reproducibility package,
...
{github.com/EkodeckStephane/fair-beth-malicious-process-detection}. The
submitted source archive includes the bibliography, IEEE class and
bibliography-style files, and every active figure required to compile the
main manuscript.

\textbf{Mandatory author declarations.}
\begin{itemize}
    \item \textbf{Originality and exclusive submission:} The manuscript is
    original, unpublished, exclusive to TDSC during review, and outside
    parallel consideration elsewhere.
    ...
    \item \textbf{Conflicts and funding:} The authors declare the absence of
    competing interests. The research was conducted with institutional
    resources; external grant funding was absent.
    \item \textbf{Ethics and data:} Human participants, animals, and sensitive
    personal data are outside the study design. The study uses publicly
    available cybersecurity datasets and reports derived experimental artifacts.
    ...
    boundaries, and audit figures are supplied as a separate four-page file,
    with independent Table S1--S3 and Figure S1--S4 numbering, consistent with
    TDSC submission requirements.
\end{itemize}
...
Atchiengang Ngueyep, St\'{e}phane Willy Mossebo Tcheunteu, Arthur Ulrich
Ewane, Ren\'{e} Ndoundam, and Julius Marcellin Nkenlifack
